...
def main(args):
    ...
    for i in range(args.epochs):
        ...
        for j,(image,target) in enumerate(train_dataloader):
            ...
            target=target.to(device) # [수정] 타겟을 디바이스로 이동
            
            # [수정] 모델에 target(레이블) 전달
            pred=model(image, noise, labels=target)
        ...
        # [수정] 원하는 숫자를 프롬프트로 생성 (예: 0~9를 순서대로 반복)
        # n_samples만큼 숫자를 생성하되, 0,1,2,3... 순서로 라벨을 만듭니다.
        context_labels = torch.arange(args.n_samples).type(torch.long) % 10
        context_labels = context_labels.to(device)

        # [수정] sampling 함수에 labels 전달
        samples=model_ema.module.sampling(args.n_samples, labels=context_labels, clipped_reverse_diffusion=not args.no_clip, device=device)
